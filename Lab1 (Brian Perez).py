...
def password_check (passwords, records):                    # passes 2 paramters, passwords list and users list (now records)
    for x in passwords :                                    # each password
        for y in range(len(records)):                       # and the length of the records so as to know how many users to check through 
            z = x + (records[y][1])                         # makes z a version of the password with the salt value attached 
            if (hash_with_sha256(z)) == (records[y][2]):    # hashes salted value and checks password correctness 
                print("Password found: ", x)                # done 
...
#    print(records[0][1])
    ...
    passwords = ["1000", "6140", "942", "6682", "2419445", "0000"]  
